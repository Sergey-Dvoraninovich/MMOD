...
def get_time_in_smo(requests, n, m):
    boundary = max(2 * n + 2 * m, int(len(state_log) * 0.2))
    total_time = 0
    total_amount = 0
    for i in range(boundary, len(requests) - boundary):
...
def count_xi_diffs(stationarity_data, n, m):
...
work_times = [900, 360, 90]
stationarity_data = get_stationarity_data(n, m, lambda_val, mu, v, work_times)
print(stationarity_data)
differences = count_xi_diffs(stationarity_data, n, m)
print(differences)
